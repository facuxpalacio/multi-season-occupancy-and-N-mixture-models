...
output:
  html_document:
    ...
    toc_depth: 3
    theme: cosmo
    highlight: tango # A pleasant syntax highlighting theme
---
...
setwd("C:/")
humm_abundance <- read.table("data_hummingbirds_lp.txt", head = T)
...
ObsCovs <- read.table("C:/RD/obs_covs_hummingbirds_lp.txt", head = T)
...
siteCovs <- read.table("C:/RD/site_covs_hummingbirds_lp.txt", head = T)
siteCovs$habitat_type <- as.factor(siteCovs$habitat_type)

# Load yearly site covariates
yearlySiteCovs <- read.table("C:/RD/yearly_site_covs_hummingbirds_lp.txt", head = T)
...
humm_abundance <- read.table("C:/RD/data_hummingbirds_lp.txt", head = T)
...
# fit_fm0 <- readRDS("C:/RD/posterior_samples_fm0.rds")
```
...
# fit_fm1 <- readRDS("C:/RD/posterior_samples_fm1.rds") # Load if not run above
traceplot(fit_fm1)
...
# fit_fm2 <- readRDS("C:/RD/posterior_samples_fm2.rds") # Load if not run above
```
...
# fit_fm3 <- readRDS("C:/RD/posterior_samples_fm3.rds") # Load if not run above
```
...
```{r best-model-plots, eval=TRUE}
# Load the best model's posterior samples
fit_fm1 <- readRDS("C:/RD/posterior_samples_fm1.rds")
ObsCovs <- read.table("C:/RD/obs_covs_hummingbirds_lp.txt", head = T)
